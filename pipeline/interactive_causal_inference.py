...
class InteractiveCausalInferencePipeline(CausalInferencePipeline):
    def __init__(
        ...
        *,
        ...
        self.semantic_bridge_tau_low = float(getattr(args, "semantic_bridge_tau_low", 0.7))
        self.semantic_bridge_tau_high = float(getattr(args, "semantic_bridge_tau_high", 0.8))
    ...
    def inference(
        ...
        *,
        ...
        for cond in cond_list:
            self._get_projected_prompt_signature(cond)
        ...
        for current_num_frames in all_num_frames:
            if next_switch_pos is not None and current_start_frame >= next_switch_pos:
                old_segment_idx = segment_idx
                completed_segment_anchor = self._build_segment_anchor_from_current_cache(cond_list[old_segment_idx])
                segment_idx += 1
                switch_mode, rho_max = self._apply_switch_memory_policy(
                    output,
                    current_start_frame,
                    cond_list[old_segment_idx],
                ...
                next_switch_pos = switch_frame_indices[segment_idx] if segment_idx < len(switch_frame_indices) else None
                print(f"[semantic-bridge] switch_mode={switch_mode}, rho_max={rho_max:.4f}, segment_idx={segment_idx}")
            ...
            if active_window_frames != -1:
                ...
                if active_window_frames != last_window_frames:
                    print(f"[adaptive-window] current_start_frame={current_start_frame}, segment_idx={segment_idx}, active_window_frames={active_window_frames}")
