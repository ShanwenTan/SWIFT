...
class CausalWanSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if kv_cache is None:
            ...
            bridge_k = kv_cache.get("semantic_bridge_k") if kv_cache is not None else None
            bridge_v = kv_cache.get("semantic_bridge_v") if kv_cache is not None else None
            bridge_scale = kv_cache.get("semantic_bridge_scale") if kv_cache is not None else None
            if bridge_k is not None and bridge_v is not None:
                if bridge_scale is not None:
                    bridge_scale = bridge_scale.to(device=bridge_k.device, dtype=bridge_k.dtype)
                    bridge_k = bridge_k * bridge_scale
                    bridge_v = bridge_v * bridge_scale
                bridge_len = bridge_k.shape[1]
            else:
                bridge_len = 0

            anchor_k = kv_cache.get("segment_anchor_k") if kv_cache is not None else None
            anchor_v = kv_cache.get("segment_anchor_v") if kv_cache is not None else None
            anchor_scale = kv_cache.get("segment_anchor_scale") if kv_cache is not None else None
            if anchor_k is not None and anchor_v is not None:
                if anchor_scale is not None:
                    anchor_scale = anchor_scale.to(device=anchor_k.device, dtype=anchor_k.dtype)
                    anchor_k = anchor_k * anchor_scale
                    anchor_v = anchor_v * anchor_scale
                anchor_len = anchor_k.shape[1]
            else:
                anchor_len = 0

            adaptive_window_tokens = kv_cache.get("adaptive_window_tokens") if kv_cache is not None else None
            effective_window_tokens = self.max_attention_size if adaptive_window_tokens is None else min(self.max_attention_size, int(adaptive_window_tokens))

            if sink_tokens > 0:
                local_budget = effective_window_tokens - sink_tokens - bridge_len - anchor_len
                ...
                if bridge_len > 0:
                    parts_k.append(bridge_k)
                    parts_v.append(bridge_v)
                if local_budget > 0:
                ...
                local_budget = max(0, effective_window_tokens - bridge_len - anchor_len)
                window_start = max(0, local_end_index - local_budget)
                k_parts = []
                v_parts = []
                if anchor_len > 0:
                    k_parts.append(anchor_k)
                    v_parts.append(anchor_v)
                if bridge_len > 0:
                    k_parts.append(bridge_k)
                    v_parts.append(bridge_v)
                k_parts.append(roped_temp_k[:, window_start:local_end_index])
                v_parts.append(temp_v[:, window_start:local_end_index])
                x = attention(roped_query, torch.cat(k_parts, dim=1), torch.cat(v_parts, dim=1))
